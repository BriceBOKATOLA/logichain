...
class RouteEntity extends BaseEntity {
  ...
  validate() {
    ...
    this.assert(
      Array.isArray(this.stops) && this.stops.length > 0,
      'La feuille de route doit contenir au moins un arrêt.',
    );
    this.assert(
      ['planned', 'in_progress', 'completed', 'validated'].includes(this.status),
      'Statut de route invalide.',
    );
  }
}
